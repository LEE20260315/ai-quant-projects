        ...
        for date in all_dates:
            ...
            for sym in symbols:
                ...
                if final_direction != 0 and final_strength > 0.05:
                    ...
                    size = max(1, int(position_value / (row['close'] * spec.multiplier)))
                    margin_needed = row['close'] * spec.multiplier * size * spec.margin_ratio
                    can_open, checks = guard.can_open(sym, final_direction, capital, margin_needed)
                    if can_open:
                        atr_stop_mult = 1.8
                        atr_take_mult = 2.5
                        if final_direction == 1:
                            sl = row['close'] - atr_val * atr_stop_mult
                            tp = row['close'] + atr_val * atr_take_mult
                        else:
                            sl = row['close'] + atr_val * atr_stop_mult
                            tp = row['close'] - atr_val * atr_take_mult
                        ...
                        guard.add_position(sym, final_direction, row['close'], size)
