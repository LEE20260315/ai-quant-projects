...
class MeanRevertSignal:
    ...
    def generate_daily_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['mr_strength'] = 0.0
        # 基于 bb_pct 和 rsi 计算精确强度, 而非统一 0.5
        if 'bb_pct' in df.columns and 'rsi' in df.columns:
            long_mask = df['mr_direction'] == 1
            short_mask = df['mr_direction'] == -1
            df.loc[long_mask, 'mr_strength'] = (0.5 + (0.05 - df.loc[long_mask, 'bb_pct']) * 3).clip(0.3, 0.9)
            df.loc[short_mask, 'mr_strength'] = (0.5 + (df.loc[short_mask, 'bb_pct'] - 0.95) * 3).clip(0.3, 0.9)
        else:
