...
def send_email(subject, html_body, attachments=None):
    if not SMTP_CONFIG['auth_code']:
        print('[MAIL] WARN: QQ_MAIL_AUTH_CODE env var not set, email not sent')
        print('[MAIL] Set it with: setx QQ_MAIL_AUTH_CODE "your_auth_code"')
        return False
    ...
    if positions:
        for sym, pos in positions.items():
            ...
            <tr>
                ...
                <td>{pos.get("entry_price", 0):.0f}</td>
