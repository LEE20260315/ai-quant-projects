...
from common.execution.risk_manager import RiskManager, AccountSnapshot, RiskConfig
...
class SignalFusion:
    ...
    def execute_signal(
        ...
        """
        SignalFusion 上的薄封装 —— 真正干活的逻辑在 execute_fused_signal.
        保留此方法是为了不破坏既有调用.
        """
        return execute_fused_signal(
            symbol=symbol,
            direction=direction,
            fused=fused,
            ...
            ctp_broker=ctp_broker,
        )


# ============================================================
# 模块级独立函数 —— 不依赖 SignalFusion 实例,
# 供 confirmation_bridge 这种没有 SignalFusion 上下文的场景使用
# ============================================================
def execute_fused_signal(
    symbol: str,
    direction: int,
    fused,
    account_state: dict,
    ...
    关键不变量（按规划要求）:
        1. 入口硬校验: can_trade() AND current_drawdown <= 0.40
        2. 风控不过 → 一律 abort
        3. 通过后才进行仓位计算与下单
    """
    # --- [1/4] 无信号直接放行 ---
    if direction == 0 or fused is None:
        return {"ok": False, "reason": "no_signal", "lots": 0}

    # --- [2/4] 强制风控校验 (Step 3 修复点) ---
    if risk_manager is None:
        logger.error("execute_fused_signal: 未注入 risk_manager, 拒绝执行以保证安全.")
        return {"ok": False, "reason": "no_risk_manager", "lots": 0}
    ...
    # 允许把 unrealized_pnl / equity_peak 注入到 positions
    unrealized = account_state.get("unrealized_pnl_by_symbol", {})
    for sym, pos in snap.positions.items():
        pos["unrealized_pnl"] = unrealized.get(sym, 0.0)
    if "equity_peak" in account_state:
        snap.positions["equity_peak"] = float(account_state["equity_peak"])

    risk_manager.attach_snapshot(snap)

    # 核心硬校验（与规划要求一字不差）
    if not risk_manager.can_trade() or risk_manager.current_drawdown > 0.40:
    ...
    if isinstance(account_state.get("positions"), dict):
        position_sizer.sync_from_positions(account_state["positions"])

    equity = float(account_state.get("current_equity", snap.capital))
    decision = position_sizer.calc_lots(
        symbol=symbol,
        account_equity=equity,
        consecutive_losses=snap.consecutive_losses,
    )
    # 叠加风控折扣
    mult = risk_manager.position_size_multiplier()
    final_lots = max(0, int(decision.lots * mult))
    if final_lots <= 0:
        return {
            "ok": False,
            "reason": f"sizer_zero:{decision.reason}",
            "lots": 0,
        }
    ...
    order_req = {
        ...
        "order_type": "limit",
        "fused": {
            "sl_atr_adj": fused.sl_atr_adj,
            "tp_atr_adj": fused.tp_atr_adj,
            "hold_days_adj": fused.hold_days_adj,
            "enhancement": fused.enhancement_applied,
        },
    }

    if ctp_broker is None:
        logger.warning("execute_fused_signal: 未注入 ctp_broker, 仅返回指令不真正下单.")
        return {"ok": True, "reason": "no_broker_dry_run", "lots": final_lots, "order": order_req}

    try:
        order_id = ctp_broker.send_order(order_req)
    except Exception as e:
        logger.exception("execute_fused_signal: 下单失败: %s", e)
        return {"ok": False, "reason": f"broker_error:{e}", "lots": 0}

    position_sizer.on_open(symbol, final_lots)
    return {
        "ok": True,
        "reason": "executed",
        "lots": final_lots,
        ...
        "sizer_reason": decision.reason,
        "risk_dd": risk_manager.current_drawdown,
    }
