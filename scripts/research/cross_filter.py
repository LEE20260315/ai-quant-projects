...
def render_markdown(candidates: list, layering: dict, top: int) -> str:
    ...
    lines.append("---")
    lines.append("")
    lines.append("## 🔄 下一步 (P0-2 自动化)")
    lines.append("")
    lines.append("1. `python scripts/research/inject_pool.py` ← 自动写入 `data/pools/2026.json`")
    lines.append("2. live_tracker.run_daily() 启动时 `load_active_pool()` 自动读池")
    lines.append("3. 每周日 20:00 Schedule 任务自动跑 inject_pool")
    lines.append("")
    return "\n".join(lines)
